...
export async function searchRepositories(query: string, sort: 'updated' | 'stars' | 'forks' = 'stars', direction: 'desc' | 'asc' = 'desc', language?: string, starsRange?: string, license?: string, updatedAfter?: string): Promise<Repository[]> {
    ...
    if (starsRange) {
      searchQuery = `${searchQuery} stars:${starsRange}`.trim();
    }
    if (license) {
      searchQuery = `${searchQuery} license:${license}`.trim();
    }
    if (updatedAfter) {
      searchQuery = `${searchQuery} pushed:>${updatedAfter}`.trim();
    }
    
    const data = await fetchGitHubAPI<{ items: Repository[] }>(`/search/repositories?q=${encodeURIComponent(searchQuery)}&sort=${sort}&direction=${direction}&per_page=30`, ['search', `search:${query}:${sort}:${direction}:${language}:${starsRange}:${license}:${updatedAfter}`]);
    return data.items;
};
